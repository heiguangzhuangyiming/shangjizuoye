...
for (i = 0; i < N; i++)
{
    stu[i].totalscore = 0.2*stu[i].usualscore + 0.2*stu[i].
                        labscore + 0.6*stu[i].finalscore;
}
for (i = 0; i < n; i++)
{
    qsort(&(stu[0]), N, sizeof(stu[0]), cmpBigtoSmall)
    for (i = 0; i < N; i++)
        printf("%d,%s,%f", stu[i].ID, stu[i].name, stu[i].
               totalscore);
}
*/
void findscore(SS stu[], int N)
{
    ...
    fprintf("%f,%f,%f,%f", stu[k].usualscore, stu[k].
            labscore, stu[k].finalscore, stu[k].totalscore);
